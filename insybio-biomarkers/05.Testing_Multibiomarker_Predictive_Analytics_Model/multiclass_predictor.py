...
class PredictMulticlass:
    ...
    def predict_fun_thread(self, patient):
        ...
        if len(pred_array) == 0:  # i.e. pred_array = [] if all models skipped and no prediction for the patient
            ...
            patient_mean = max(set(pred_array), key=list(pred_array).count)
...
def predictor(dataset_initial, features, model_list, model_feature_file, classification_chain,
    ...
    try:
        ...
        prediction = PredictMulticlass(data_tran, model_list, features_df.values.tolist(), False, dict_chain,
...
def main(filename, var_dict, thread_num, user='unknown', jobid=0, pid=0):
    ...
    predict = PredictMulticlass(data_tran, model_list, features_df.values.tolist(), False, dict_chain, data_org_pred,
