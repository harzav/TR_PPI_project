'''
regression_predicting_test_set.py

A script to predict the labels of a test set based on a given model produced by the modeller on a training set.
...
Input:
       ...
       length_of_features_filename (String): the f ilename with the length of the features of the training set, taken from step 02.
...
python3.6 regression_predicting_test_set.py Input/regr/test_dataset.txt Input/regr/test_labels.txt Input/regr/maximums.txt Input/regr/minimums.txt Input/regr/averages_for_missing_values_imputation.txt Input/regr/features_list.txt 1 1 Input/regr/model_2018_10_09 0 "" Input/twoclass/new/markers.txt 8 0 0 Input/regr/training_labels.txt Input/regr/length_of_features.txt length_of_features.txt "Feature_0,Feature_1"
'''
...
def geometric_mean_normalization(dataset_imputed, selected_features, output_message):
    """
    It does a normalization of the data based on the geometric mean.

    Args:
        dataset_imputed (list of lists): the initial dataset
        selected_features (list): the names (or lines) of the selected genes that with which the geometric mean will be calculated
...
def predictor(testset_filename, testset_labels_filename, maximums_filename, minimums_filename, averages_filename, features_filename,
    ...
    try:
        ...
        #testSet = list(map(list, zip(*testSet)))
        ...
        if testset_labels_filename:
            ...
            #testSet_labels.pop() # last element is a string ''
            testSet_labels = list(map(float, testSet_labels))
        else:
            ...
            training_labels = [float(x) for x in training_labels]
            a = min(training_labels)
            b = max(training_labels)
            testSet_labels = [uniform(a,b) for _ in range(len(testSet[0]))]
        ...
        if data_been_preprocessed_flag == 0:
            ...
            if variables_for_normalization:
                ...
                if isinstance(variables_for_normalization, list):
                    ...
                    for variable in variables_for_normalization:
                        try:
                            ...
                            #return [0, "The biomarker(s) provided are not in the list of the biomarkers of the input file ! Try again ! {}".format(e)]
                            pass
                ...
                if variables_for_normalization_nums:
                    ...
                    #print_data_with_markers_and_samples(dataset_imputed, markers, samples, output_folder, "dataset_normalized_mvimputed_geo.txt")
    ...
    try:
        outputs = testSet_labels

        testing_inputs = inputs
        testing_outputs = outputs
        testing_outputs = [float(x) for x in testing_outputs]

        # Loading the model
        model = svm_load_model(model_filename)

        testing_inputs = list(map(list, zip(*testing_inputs)))
        [result_label, mse_accuracy, dec_values] = svm_predict(testing_outputs, testing_inputs, model)

        if testset_labels_filename:
            logging.info('Full Training Set Root Mean Square Error:' + str(mse_accuracy))
            logging.info("Successfully finished!")
            with open(output_folder + "metrics.txt","w") as metrics_handle:
                metrics_handle.write("Test set mean squared error: " + "{0:.2f}".format(mse_accuracy[1]) + "\n")
                metrics_handle.write("Test set squared correlation coefficient: " + "{0:.2f}".format(mse_accuracy[2]) + "\n")

        # Saving result labels
        result_labels_output = copy.deepcopy(result_label)
