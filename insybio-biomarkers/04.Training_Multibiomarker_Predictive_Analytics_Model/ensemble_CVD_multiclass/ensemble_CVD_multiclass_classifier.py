...
class FeatureSelection:
    ...
    def __init__(self, dataset, proteins, min_values, max_values, class_Labels, evaluate_values, max_trees, population,
        ...
        self.pathFeature = ''
        self.pathModel = ''
        self.num_threads = int(thread_num) - 1 if int(thread_num) > 2 else 1
    ...
    def train_Classifier(self, dataTran, final_selected, classifier_type, C_val, Gamma_val, TreeCount,
        ...
        # Sum_neurons = 0.0001
        if classifier_type < 1.0:  # linear svm
            clf = svm.SVC(C=C_val, kernel='linear', probability=True)  # probability to compute ROC
        elif 1.0 <= classifier_type < 2.0:  # RBF svm
            clf = svm.SVC(C=C_val, gamma=Gamma_val, kernel='rbf', probability=True)
        elif 2.0 <= classifier_type < 3.0:  # random forest, n_jobs=-1 for parallelsim
            clf = RandomForestClassifier(n_estimators=int(TreeCount))
        ...
        if classifier_type < 3.0:
            ...
            cv_scores = cross_validate(clf, inputs, outputs, cv=skf, scoring=self.score_types)
            f2_score = cross_val_score(clf, inputs, outputs, cv=skf, scoring=ftwo_scorer)
        else:
            # 3d vectors
            inputs_neural = np.reshape(inputs, (inputs.shape[0], inputs.shape[1], 1))
    ...
    def biomarker_feature_select(self):
        ...
        for rep in range(generations):  # iterate on generations
            ...
            goal_values = Parallel(n_jobs=self.num_threads)(
...
def main(varDict, user='unknown', jobid=0, pid=0):
    ...
    max_values = [3.999, 500.0, 500.0, 2.0, 0.5, 30.0, 4, 1.0, 100.0, 0.05, 200, 128, 4, 5, 4, 0.5]
...
def main_only_training(outfolder, dataset, labels, features, population, generations, folds, mutation_prob,
    ...
    min_values = [0.001, 0.001, 0.001, 0.001, 0.0001, 1.0, 2.0, 0.0, 10.0, 0.001, 50, 32, 0, 0, 0, 0.2]
    max_values = [2.999, 500.0, 500.0, 2.0, 0.5, max_kbestvalue, 5.0, 1.0, 100.0, 0.05, 200, 128, 4, 5, 4, 0.5]
    # min_values=[0.001,0.001,0.001,0.001,0.0001,50.0,5.0, 0.0,20.0] # for testing; single k and feature
    # max_values=[3.0,1000.0,1000.0,4.0,0.5,51.0,6.0, 0.0,30.0] # for testing; single k and feature
    goal_values = [0.0001 for _ in range(11)]
