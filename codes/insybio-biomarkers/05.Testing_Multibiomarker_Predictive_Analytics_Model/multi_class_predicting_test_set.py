'''
multi_class_predicting_test_set.py
...
python3.6 multi_class_predicting_test_set.py Input/multi/test_dataset.txt Input/multi/test_labels.txt  Input/multi/maximums.txt Input/multi/minimums.txt Input/multi/averages_for_missing_values_imputation.txt  Input/multi/features_list.txt 1 1 Input/multi/model_2018_10_09 0 "" Input/twoclass/new/markers.txt 8 0 0  Input/multi/training_labels.txt Input/multi/length_of_features.txt "Feature_0,Feature_1"
'''
...
def transform_labels_to_numeric(labels, unique_labels):
    """
    Transforms alphanumeric labels to numeric ones in the case of multiclass classification.
...
def transform_labels_to_alpha(labels, unique_labels):
    ...
    Returns:
        mylist: the decoded labels with alphanumeric names
    """
    mylist = []
    for x in labels:
        for i in range(len(unique_labels)):
            if x == i:
                mylist.append(unique_labels[i])
...
def predictor(testset_filename, testset_labels_filename, maximums_filename, minimums_filename, averages_filename, features_filename,
    ...
    try:
        ...
        if testset_labels_filename:
            ...
            testSet_labels = [random.choice(unique_labels) for _ in range(len(testSet[0]))]
